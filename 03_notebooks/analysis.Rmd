...
tab_ats_rmsd_quantiles <- table_nums(
  ...
  caption = "Percentiles of the absolute deviation between EWAC and GF schedule (n=22,373)")
...
- the root mean squared deviation $\text{RMSD} = \sqrt{n^{-1} \sum_{i=1}^{n}{( \rm{EWAC_i} - \rm{GF_i} )^2}}$ can be assimilated to a measure of accuracy or total error: it capture both bias and random deviation from the assumed gold standard. For example, an RMSD of 2 signifies that the EWAC is on average with +/-2 UK units of the gold standard. In this study, we test the hypothesis that the RMSD is greater than 2 UK units using a one-sided $\chi^2$ homogeneity test.

Second, we test the hypothesis that the EWAC's validity varies across population subgroups by regressing both (a) the simple deviation in a linear model and (b) the root squared deviation in a log-transformed linear model. Both models include the following predictors: sex by age group; ethnic group; favourite drink (beer; wine; spirits alone; mixed spirits; cider; other); highest educational qualification; religion; smoking status; recent attempts at reducing or stopping drinking.
...
ats$f_ageg <- relevel(ats$ageg7, ref = "25-34 years")
# lm_bias_mp <- lm((ewac_mp-gfmeanweekly) ~ sex*f_ageg + ethgrp + favdrink + 
#                    tryalclyc2 + religion + highqual  + smokstat, data = ats)
# lm_rmsd_mp <- lm(sqrt((ewac_mp-gfmeanweekly)^2) ~ sex*f_ageg + ethgrp + favdrink + 
#                    tryalclyc2 + religion + highqual  + smokstat, data = ats)
lm_bias_stan <- lm((ewac_qfv-gfmeanweekly) ~ sex*f_ageg + ethgrp + favdrink + 
                   tryalclyc2 + religion + highqual  + smokstat, data = ats)
lm_rmsd_stan <- lm(log((ewac_qfv-gfmeanweekly)^2) ~ sex*f_ageg + ethgrp + favdrink + 
                   tryalclyc2 + religion + highqual + smokstat, data = ats)
...
ggplot(ewac_bias_plot, aes(x = factor(x), y = MD)) +
  ...
  geom_segment(aes(x = "0", xend = "0", y =0, yend = 3.5), 
               lwd = 1, arrow = arrow(length = unit(0.03, "npc"))) +
  geom_label(aes(x = "0", y = 3.8, label = "EWAC overestimates consumption"),
             hjust = "outward", colour = "purple") +
  theme_minimal() 
...
Error is significantly greater in mean (those reporting any religion other than Christian or Muslim; and in current smoker compared to never-smokers. In addition, respondents who attempted to reduce their alcohol consumption in the last year also had a higher RMSD. On the contrary, educational qualifications seem to significantly improve the agreement between EWAC and the gold standard. The RMSD is reduced by an average 2 units with an educational qualification compared to no qualifications (and without effect on MD), suggesting that respondents may have better recall and clarity over alcohol beverage content.



```{r ewac_subgroup_error, fig.width=7, fig.height=3}
...
ggplot(ewac_error_plot, aes(x = factor(x), y = MSD_ratio)) +
  scale_x_discrete(breaks = names(labs2), 
                   labels = labs2, name = NULL)+
  scale_y_continuous(breaks = seq(0, 4, .5),
                     "Relative MSD")+
  coord_flip() +
  geom_text(aes(label = round(MSD_ratio, 1)), vjust = 1.2) +
  geom_linerange(aes(ymin = MSD_ratio_LB, ymax = MSD_ratio_UB), colour = "purple") +
    geom_point() +
  geom_hline(aes(yintercept = 1)) +
  geom_segment(aes(x = "00", xend = "00", y =1, yend =1.6), 
               lwd = 1, arrow = arrow(length = unit(0.03, "npc"))) +
  geom_label(aes(x = "00", y = 1.1, label = "Worse agreement of EWAC\n with gold standard"),
             hjust = "inward", vjust= -0.2, colour = "purple") +
  theme_minimal() 
```

Ratio of subgroup RMSD to reference category RMSD
