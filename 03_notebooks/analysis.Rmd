...
fig_scatter_ats <- fig_nums(
  ...
  caption = 'Plots of EWAC against GF in (a) ATS participants and (b) low/increasing risk ATS participants')
...
- the mean deviation $\text{MD} = n^{-1} \sum_{i=1}^{n}{(\rm{EWAC_i} - \rm{GF_i} )}$ can be assimilated to a measure of bias under the assumption that GF is a gold standard. In this study, we test whether the MD is greater than 1 UK unit using a two-sided $t$-test. 
- the root mean squared deviation $\text{RMSD} = \sqrt{n^{-1} \sum_{i=1}^{n}{( \rm{EWAC_i} - \rm{GF_i} )^2}}$ can be assimilated to a measure of total error: it capture both bias and random deviation from the assumed gold standard. In this study, we test the hypothesis that the RMSD is greater than 2 UK units using a one-sided $\chi^2$ homogeneity test.

Second, we test the hypothesis that the EWAC's validity varies across population subgroups by regressing both (a) the simple deviation and (b) the root squared deviation in linear regression models, against the following predictors: sex by age group; ethnic group; favourite drink (wine, beer, etc.); highest educational qualification; religion; smoking status; general health; recent attempts at reducing or stopping drinking.
...
The first step involved choosing a set of coefficients to compute the EWAC. We compare using the midpoint of the AUDIT item intervals; and coefficients estimated using a statistical model (see supplementary materials 1). 

The EWAC based on midpoint coefficients produces a mean deviation (MD) of `r tbrounding(test_midpoint$MD, 2)` alcohol units, indicating a bias inferior to +/- 1 UK unit (`r pvrounding(test_midpoint$MD.p)`). The root mean squared deviation (RMSD) is estimated at `r tbrounding(test_midpoint$RMSD, 2)`, significantly greater than 4 UK units (`r pvrounding(test_midpoint$RMSD.p)`), suggesting that the EWAC of any given individual would be on average 12 units from the truth (treating GF schedule can be used as a gold standard). 

Coefficients estimated by the statistical model provide a small improvement with MD = `r tbrounding(test_stan$MD, 2)` (`r pvrounding(test_stan$MD.p)`). Error remains statistically significantly greater than 4 with  RMSD = `r tbrounding(test_stan$RMSD, 2)` (`r pvrounding(test_stan$RMSD.p)`). [Figure 1](#figure1) compares individual EWAC and GF values. We note the departure of lines of best fit from the diagonal, whether estimated using ordinary least squares or using a thin plate regression spline. This discrepancy is more severe beyond 25 units per week.
