...
tab_ats_rmsd_stratified <- table_nums(
  name = "tab_ats_rmsd_stratified",
  caption = paste0("Root Mean Squared Deviation (RMSD) between EWAC and GF schedule by alcohol consumption bracket (n = ", formatbm(valid_n_study1a_validation), ")"))
...
2. **AUDIT score interpretation:** The ordinal scores produced by the AUDIT-C (range: 0–12) and the full AUDIT (range: 0–40) are multidimensional measures of alcohol risk. To date, most brief intervention models involve dichotomising AUDIT scores, on the basis of complex diagnostic accuracy studies [@DeMeneses-Gaya2009], at cut-offs that vary internationally [@Nadkarni2019]. In practice, this may contribute to healthcare professionals lacking confidence in discussing alcohol risks and consumption [@Beich2002; @Johnson2010; @Hutchings2006; @McCormick2006], and needing to be trained to deliver feedback [@Beich2002; @Johnson2010]. Evidence also suggests that patients' understanding of alcohol risks overlaps loss of control more than alcohol consumption [@Khadjesari2018; @ODonnell2020]. In response, some academic models of alcohol care advocate the framing of brief interventions around the continuum of alcohol use [@Nutt2014] rather than thresholds, since these can trigger stigma related to loss of control [@Rehm2013a].
...
However, there was substantial variation in RMSD; in 50% of participants, the EWAC fell within $\pm$ 2.1 UK units of the GF weekly consumption estimate. RMSD was proportional to alcohol consumption, amounting to about 50% of the EWAC value (`r table_nums('tab_ats_rmsd_stratified', display = "cite")`). Thus, an interval defined as the EWAC $\pm$ 50% (e.g. '2–6 units/week' for an EWAC of 4; '10–30 units/week' for an EWAC of 20) contained the reference standard for over half (58%) of individuals.

`r table_nums('tab_ats_rmsd_stratified')`

```{r rmsd_stratified}
rmsd_stratified <- ats %>% 
  filter(!is.na(gfmeanweekly) & !is.na(ewac_qfv)) %>%
  mutate(
    `EWAC value (UK units/week)` = cut(
      ewac_qfv,
      breaks = c(0, 5, 10, 20, 30, 45, 60, 75, 100, 150, 400),
      right = F
    ),
    ewac_lower = ewac_qfv - ewac_qfv*.5,
    ewac_upper = ewac_qfv + ewac_qfv*.5)

bind_rows(
  rmsd_stratified %>% 
    group_by(`EWAC value (UK units/week)`) %>%
    summarise(
    n = formatbm(n()),
    `RMSD [95% confidence interval]` = paste0(
      round(sqrt(mean((ewac_qfv - gfmeanweekly)^2, na.rm = T)), 1), " [",
      round(sqrt( mean( (ewac_qfv - gfmeanweekly)^2 ) -
                    1.96 * sqrt(var( (ewac_mp - gfmeanweekly)^2 ) / ( n() - 1 ) )), 1)
      ,"–", 
      round(sqrt( mean( (ewac_qfv - gfmeanweekly)^2 ) + 
                    1.96 * sqrt(var( (ewac_mp - gfmeanweekly)^2 ) / ( n() - 1 ) )), 1)
      , "]"
    ),
    `Participants with GF contained in \n[EWAC*0.5; EWAC*1.5] interval (%)` = paste0(
      formatbm(
      sum( gfmeanweekly >= ewac_lower & gfmeanweekly <= ewac_upper)),
      " (", round(
      mean( gfmeanweekly >= ewac_lower & gfmeanweekly <= ewac_upper)*100, 1), ")"
    )
  ),
  rmsd_stratified %>% 
    summarise(
    `EWAC value (UK units/week)` = "All values",
    n = formatbm(n()),
    `RMSD [95% confidence interval]` = paste0(
      round(sqrt(mean((ewac_qfv - gfmeanweekly)^2, na.rm = T)), 1), " [", 
      round(sqrt( mean( (ewac_qfv - gfmeanweekly)^2 ) -
...
Being equivalent to the AUDIT-C in speed and international standardisation, the EWAC may be suitable for use in clinical practice to support brief interventions and to feed back a reliable interval estimate of alcohol consumption (eg: '6–18 units/week' or '50–140g/week'). The EWAC is available as a web app at <https://ewac.netlify.app> along with resources to facilitate implementation (R software package, spreadsheets).

Assessment of alcohol consumption is not well embedded in clinical practice [@Browne2016]. The EWAC calculator fills a gap in resources by transforming the answers from the Extended AUDIT-C into a direct estimate of an individual's weekly alcohol consumption. This is a more directly accessible metric which should facilitate behaviour change by empowering people to monitor and control their alcohol consumption with--or without--the involvement of healthcare professionals, and should be assessed in future evaluations.

Nutt, Rehm *et al.* [@Nutt2014; @Rehm2016] argued that alcohol-related harm is best prevented if individuals know their consumption level, and health professionals in all settings can engage patients effectively to manage risks with evidence-based interventions, in a similar way to other risk factors for disease, for example blood pressure or cholesterol. Yet, knowledge of beverages' alcohol content is generally poor [@Mongan2015], and a survey evaluating the 2016 change in UK alcohol guidelines found that just 8% of the UK drinkers knew the new recommended limits [@Rosenberg2018]. The EWAC can support interventions focused on recognising the alcohol content/volume of drinks, and recommended low-risk limits.

In addition, the EWAC's dimensional rather than categorical format can be useful to position recipients of brief interventions on the continuum of alcohol use [@Rehm2013a; @Rehm2014a], which may reduce the stigma of loss of control associated with screening-based interventions [@Khadjesari2018; @ODonnell2018]. It can act as a complement, rather than a substitute to the multidimensional quality of AUDIT-C or the full AUDIT.
...
```{r missingness_tests, results='asis', eval=FALSE}
...
```{r missingness_pattern,  fig.height=5, fig.width=25, eval=FALSE}
