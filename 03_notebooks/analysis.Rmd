...
Next, the MD and RMSD are regressed against respondents characteristics in order to identify subgroups with potentially stronger bias or error ([Table 3](#tab_ewac_validity_reg)). Models suggest that the factors account for a negligible proportion of both MD and RMSD (<2%). Despite this: 
...
mhosp <- glm(round(alcunits) ~ qfv_log + q_admin_mode+
                 week_since + tlfb_weekday, family = poisson, data = tdata_b)

# require(pscl)
# mzip <- zeroinfl(round(alcunits) ~ qfv_log + q_admin_mode +
...
tlfb_measured <- tdata_b %>% 
  filter(days_since <= 28 & !is.na(alcunits) & !is.na(ewac_qfv)) %>% 
  dplyr::select(pid, q_admin_mode, ewac_qfv, alcunits)

tlfb_imputed <- tdata_b %>% 
  filter(days_since <= 28 & is.na(alcunits) & !is.na(ewac_qfv)) 
tlfb_imputed$alcunits <-  predict(mhosp, tlfb_imputed, type = "response")

tlfb_person <- dplyr::select(tlfb_imputed, pid, q_admin_mode, ewac_qfv, alcunits) %>% 
  bind_rows(., tlfb_measured) %>% 
  group_by(pid, q_admin_mode, ewac_qfv) %>% 
  summarise(tlfb_units = sum(alcunits)/4)
  
tlfb_descriptives <- list(
  MD = mean(tlfb_person$ewac_qfv - tlfb_person$tlfb_units),
  MD.p = t.test(tlfb_person$ewac_qfv - tlfb_person$tlfb_units)$p.value,
  RMSD = sqrt(mean((tlfb_person$ewac_qfv - tlfb_person$tlfb_units)^2, na.rm = T)),
  RMSD.p = chisq.test.variance( (tlfb_person$ewac_qfv - tlfb_person$tlfb_units)^2,
                                sigma = 4)$p.value
)
```
...
ggplot(tlfb_person, aes(y = ewac_qfv, x = tlfb_units)) +
  geom_point() +
  geom_abline(aes(intercept = 0, slope = 1, col = "Identity", linetype = "Identity")) +
  geom_smooth(aes(col="OLS", linetype = "OLS"), method ='lm', formula=y~x) +
  geom_smooth(aes(col="Spline", linetype = "Spline"), 
