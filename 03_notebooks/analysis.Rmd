...
where $b$ denotes the average number of units of alcohol consumed in an intense drinking day. Coefficients $F, Q, V$ and $b$ are unknown and are estimated in two ways:
...
The ATS data is strongly affected by data missingness: a high proportion of respondents (`r tbrounding(mean(ats$audit1 == 1)*100)`%, $n$ = `r formatbm(sum(ats$audit1 == 1))`) reported never drinking alcohol in AUDIT item 1 and were not asks any further questions about alcohol consumption. They were thus excluded from the analysis, despite many occasional drinkers amongst them. A further `r formatbm(sum(ats$audit1 != 1 & is.na(ats$gfmeanweekly)))` respondents (`r tbrounding(sum(ats$audit1 != 1 & is.na(ats$gfmeanweekly))/sum(ats$audit1 != 1))`% of those reporting drinking in AUDIT item 1) did not have a valid GF alcohol consumption record and were also excluded. Missing data in the ATS' GF schedule is assumed to be missing at random conditionally on the Extended AUDIT-C responses. We employ a Bayesian hierarchical fitted in STAN on the remaining sample ($n$ = `r formatbm(nrow(na.omit(select(ats, audit1, audit2, audit3, gfmeanweekly))))`) with bell-shaped beta priors on coefficients $F, Q, V$ bounded by their nominal values, and an exponential prior on $b$ with a lower bound of 5.5 units. Further detail on the modelling is available in supplementary materials.

Next, we quantify the deviation between the EWAC and the GF schedule.  Succintly, this analysis treats GF like a gold standard. It enables the examination of two types of deviations across every participant $i$ in the ATS sample:
...
Using midpoint coeffients, the analysis reveals that the coefficients are invalid. With a mean deviation (MD) of `r tbrounding(test_midpoint$MD, 2)` alcohol units (`r pvrounding(test_midpoint$MD.p)`), the EWAC shows a slight positive bias. The root mean squared deviation (RMSD) is estimated at `r tbrounding(test_midpoint$RMSD, 2)` (`r pvrounding(test_midpoint$RMSD.p)`), suggesting that the EWAC of any given individual would be on average 12 units from the truth (treating GF schedule can be used as a gold-standard). Using coefficients estimated by the statistical model, bias remains statistically significant with MD = `r tbrounding(test_stan$MD, 2)` (`r pvrounding(test_stan$MD.p)`) and RMSD = `r tbrounding(test_stan$RMSD, 2)` (`r pvrounding(test_stan$RMSD.p)`).
...
```{r warning=F}
select(ats, gfmeanweekly, ewac_qfv) %>% 
  na.omit %>% 
  ggplot(aes(x = gfmeanweekly, y = ewac_qfv)) +
  geom_point(alpha = .2) +
  geom_abline() +
...
require(memisc)
ewac_reg_results <- mtable('MP bias' = lm_bias_mp,
            'MP RMSE' = lm_rmse_mp,
            'Stan bias' = lm_bias_stan,
            'STAN RMSE' = lm_rmse_stan,
            summary.stats = c('sigma', 'R-squared','F','p','N'))
# tab_model(lm_bias_mp,lm_rmse_stan, file="output_lm_bias.html")
# htmltools::includeHTML("output_lm_bias.html")
library(pander)
pander( ewac_reg_results, style='rmarkdown' )
...
ggplot(scatterdata, aes(ewac_qfv, tlfb_units)) +
  geom_point() +
  geom_abline() +
  geom_smooth(method='lm',formula=y~x) +
  theme_bw() +
  ...
  # coord_
  scale_x_continuous(breaks = seq(0, 100, 2), minor_breaks = F) +
  scale_y_continuous(breaks = seq(0, 100, 2), minor_breaks = F) +
  geom_hline(yintercept = 14, color = "grey50") + 
  geom_vline(xintercept = 14, color = "grey50")
