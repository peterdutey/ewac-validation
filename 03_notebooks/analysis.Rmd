...
ats <- ats[ats$actage >= 18 & ats$sex != "In another way",]
...
## Study 1: Concurrent consistency with Graduated Frequency in England

Previous research on quantity-frequency alcohol instruments suggests that the product of the quantity and frequency questions in the AUDIT-C (see AUDIT-1 and AUDIT-2 in [Table 1](#table1)) could be used to produce a measure of usual consumption [@Lemmens1992].

![Quantity-Frequency-Variability estimator overview (adapted from @Lemmens1992)](qfv.png)

To replicate this approach, we determine weights to be applied to each Extended AUDIT-C  response item in order to compute the EWAC.  

- using the AUDIT-C response item interval midpoint: for example, 2 to 3 times per week is set to 2.5 times per week
- using a statistical model to estimate the coefficients directly from the data. Missing data in the GF schedule (14.7% of participants giving a non-'Never' answer to the AUDIT-C item 1) is assumed to be missing at random conditionally on the Extended AUDIT-C responses. We employ a Bayesian hierarchical fitted in STAN setting uniform priors on the value of each AUDIT-C item. Detail is available in supplementary materials.
...
```{r}
ats$ewac_stan <- proc_EWAC(ats[,c("audit1_label", "audit2_label", "audit3_label")],
...
ats$ewac_v3<- proc_EWAC(ats[,c("audit1_label", "audit2_label", "audit3_label")],
                           audit_coef = audit_weights_version4 , method = "qfv")
ats %>% 
  select(gfmeanweekly, ewac_v3) %>% na.omit %>% 
ggplot(aes(gfmeanweekly, ewac_v3)) +
  geom_point() +
  geom_abline() +
  ...
  ggtitle("Preliminary STAN estimates") +
...
```{r}
lm1 <- lm((ewac_v3-gfmeanweekly)  ~ ageg*sex + favdrink + ethgrp, data = ats)
summary(lm1)
```

```{r}
lm1 <- lm(sqrt((ewac_v3-gfmeanweekly)^2) ~ ageg*sex + favdrink + ethgrp, data = ats)
summary(lm1)
```
