...
select(ats, gfmeanweekly, ewac_qfv) %>% 
  na.omit %>% 
  ggplot(aes(gfmeanweekly, ewac_qfv)) +
  geom_point(alpha = .2) +
  geom_abline() +
...
*Table 3:* <a name="table3"></a>*Coefficients of linear regression of EWAC mean deviation from GF*
...
ggplot(scatterdata, aes(ewac_qfv, tlfb_units)) +
  ...
  geom_smooth(method='lm',formula=y~x) +
  theme_bw() +
  xlab("EWAC (UK units/week)") +
  ylab("Crude TLFB sample mean (UK units/week)")+
