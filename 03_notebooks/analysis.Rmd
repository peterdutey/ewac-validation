...
valid_n_study1b <- nrow(na.omit(select(ats, audit1_label, audit2_label, audit3_label, gfmeanweekly,
                                      sex, ageg7, ethgrp, religion, highqual, smokstat)))
valid_n_study1c <- nrow(na.omit(select(
...
This study collected TLFB daily alcohol consumption from a total of `r sum(!is.na(qdata$auditc_score))` participants, of whom 63 were classified as low-risk drinkers, 25 as increasing risk alcohol users (AUDIT-C score of 5 to 8), and 17 as high-risk alcohol users (AUDIT-C score>=8). A total of `r as.numeric(summarise(filter(tdata_b, is.na(alcunits)), n_distinct(pid)))` participants did not provide TLFB information for at least one day missing out of 28, resulting in `r round(mean(is.na(tdata_b$alcunits))*100, 0)`% (`r sum(is.na(tdata_b$alcunits))`/`r nrow(tdata_b)`) of TLFB days were missing.

To impute those, as well as study the association with the EWAC and the Extended AUDIT-C's mode of administration, a Poisson model is estimated (`r table_nums("tab_hosp_validity_reg", display="cite")`). This model suggests that the EWAC underestimates alcohol consumption, with  1.18*** (1.11, 1.25)
[95% CI: 3.05; 3.49]. Self-administration of the Extended AUDIT-C may be associated with some underreporting: coefficients suggest that reports of daily alcohol consumed are 15% [5; 26%] higher than the self-administered EWAC, compared to the researcher-administered EWAC.

```{r }
tdata_b$ewac_stan_log <- log(tdata_b$ewac_stan/7)
tdata_b$ewac_mp_log <- log(tdata_b$ewac_mp/7)

mhosp <- glm(round(alcunits) ~ ewac_stan_log*q_admin_mode +
...
A Poisson model is estimated both to impute individual To estimate comparable measures of deviation as in previous sections, the model is used to impute the `r nrow(tlfb_imputed)` out of `r formatbm(nrow(tlfb_person))` daily consumption figures that were missing in total for `r length(unique(tlfb_imputed$pid))` participants. In this small pilot dataset, MD = `r tbrounding(tlfb_descriptives$MD)` unit is measured (`r pvrounding(tlfb_descriptives$MD.p)`) while RMSD = `r tbrounding(tlfb_descriptives$RMSD)` [95%CI: `r tbrounding(sqrt(tlfb_descriptives$MSD + tlfb_descriptives$MSD.ME))`; `r tbrounding(sqrt(tlfb_descriptives$MSD - tlfb_descriptives$MSD.ME))`] and is statistically significantly greater than 2 units (`r pvrounding(tlfb_descriptives$RMSD.p)`). A potential reason for RMSD is 
