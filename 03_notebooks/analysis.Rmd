...
# table(ats$test_best, ats$test_qf <= ats$test_qfv)
MD_midpoint <- mean(ats$ewac_mp - ats$gfmeanweekly, na.rm = T)
RMSD_midpoint <- sqrt(mean((ats$ewac_mp - ats$gfmeanweekly)^2, na.rm = T))
MD_stan_qfv <- mean(ats$ewac_mp - ats$gfmeanweekly, na.rm = T)
RMSD_stan_qfv <- sqrt(mean((ats$test_qfv - ats$gfmeanweekly)^2, na.rm = T))

MD_stan_qfv_men <- ats %>% 
  filter(sex == "Men") %>% 
  summarise(MD_stan_qfv_men = mean(test_qfv - gfmeanweekly, na.rm = T)) %>% 
  as.numeric(.$MD_stan_qfv_men)
...
Using midpoint coeffients, the analysis reveals that the coefficients are invalid. With a mean deviation (MD) of `r tbrounding(MD_midpoint, 2)`, the EWAC shows a positive bias by 0.7 unit. The root mean squared deviation (RMSD) is estimated at `r tbrounding(RMSD_midpoint, 2)`. 

Using modelled coefficients, MD `r tbrounding(MD_stan_qfv, 2)` and RMSD of `r tbrounding(RMSD_stan_qfv, 2)`.

With early signs that validity was strongly dependent on gender, we estimated coefficients independently for each gender to test for the use of gender-specific coefficients. This analysis was not planned in the pre-registered protocol but appeared to be justified by differences in alcohol consumption between women and men.  

`r tbrounding(MD_stan_qfv_men)`
...
```{r}
library(sjPlot)
lm_bias_mp <- lm((ewac_mp-gfmeanweekly)  ~ sex*ageg + favdrink + ethgrp, data = ats)
lm_bias_stan <- lm((test_qfv-gfmeanweekly)  ~ sex*ageg + favdrink + ethgrp, data = ats)
...
lm_rmse_mp <- lm(sqrt((ewac_mp-gfmeanweekly)^2) ~ sex*ageg + favdrink + ethgrp, data = ats)
lm_rmse_stan <- lm(sqrt((test_qfv-gfmeanweekly)^2) ~ sex*ageg + favdrink + ethgrp, data = ats)
