...
tab_study1_roc <- table_nums(
  name = "tab_study1_roc",
  caption = "Receiver operating characteristics of AUDIT-C score and EWAC for consumption >= 14 UK units/week")
...
Coefficients estimated by the statistical model provide a small improvement with MD = `r tbrounding(test_stan$MD, 2)` (`r pvrounding(test_stan$MD.p)`). Error remains statistically significantly greater than 4 with  RMSD = `r tbrounding(test_stan$RMSD, 2)` (`r pvrounding(test_stan$RMSD.p)`). `r fig_nums("fig_scatter_ats", display = "cite")` compares individual EWAC and GF values. We note the departure of lines of best fit from the diagonal, whether estimated using ordinary least squares or using a thin plate regression spline. This discrepancy is more severe beyond 25 units per week.
...
Next, the MD and RMSD are regressed against respondents characteristics in order to identify subgroups with potentially stronger bias or error (`r table_nums("tab_ewac_validity_reg", display = "cite")`). $R^2$ statistics suggest that the factors account for a very modest proportion of both MD and RMSD (<2%). Despite this:
